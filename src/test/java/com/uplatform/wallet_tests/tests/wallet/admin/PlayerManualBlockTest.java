...
/**
 * Интеграционный тест, проверяющий позитивный сценарий ручной блокировки игрока через CAP API:
 * {@code PATCH /_cap/player/api/v1/admin/players/{playerId}/properties}.
 *
 * <p><b>Идея теста:</b> Гарантировать абсолютную надежность и сквозную консистентность одной из самых критичных
 * функций безопасности — полной блокировки аккаунта игрока. Тест подтверждает, что команда, отданная через единый
 * административный интерфейс (CAP API), безошибочно распространяется по всей микросервисной архитектуре.
 * Это обеспечивает немедленное прекращение всей активности игрока и синхронизацию его заблокированного статуса
 * во всех зависимых системах, исключая любые расхождения в данных.</p>
 *
 * <p><b>Сценарий тестирования:</b></p>
 * <p>Тест проверяет полный цикл ручной блокировки игрока, инициированной администратором, и отслеживает
 * распространение этого статуса по ключевым компонентам системы.</p>
 *
 ...
 *   <li>Проверка конечного состояния в Redis, чтобы убедиться, что агрегат кошелька отражает статус блокировки.</li>
 * </ol>
 *
 * <p><b>Ожидаемые результаты:</b></p>
 * <ul>
 *   <li>Запрос на блокировку игрока через CAP API выполняется успешно (HTTP 204 NO CONTENT).</li>
 *   <li>В Kafka топике {@code player.v1.account} появляется событие о смене статуса игрока на {@code BLOCKED}.</li>
 *   <li>В NATS публикуется событие о блокировке кошелька ({@code wallet_blocked}).</li>
...
class PlayerManualBlockTest extends BaseTest {
    ...
    @Test
    @DisplayName("CAP: Ручная блокировка игрока - Создание.")
