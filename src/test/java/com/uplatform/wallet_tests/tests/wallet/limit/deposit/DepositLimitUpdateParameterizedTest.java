...
/**
 * Проверяет возможность уменьшить лимит на депозит.
 *
 * Создаётся Deposit лимит через Public API, затем PATCH-запросом его сумма
...
public class DepositLimitUpdateParameterizedTest extends BaseParameterizedTest {
    ...
    @DisplayName("Изменение deposit лимита в меньшую сторону")
    void updateDepositLimitAmount(NatsLimitIntervalType periodType) {
        ...
        step("Public API: Получение лимита Deposit и его валидация после обновления", () -> {
            ...
            assertAll("fapi.get_deposit_limits.limit_content_validation",
                    () -> assertEquals(ctx.updateEvent.getPayload().getLimits().get(0).getExternalId(), fapiLimit.id(), "fapi.get_deposit_limits.limit.id"),
                    ...
                    () -> assertEquals(0, newAmount.compareTo(fapiLimit.rest()), "fapi.get_deposit_limits.limit.rest_equals_amount"),
