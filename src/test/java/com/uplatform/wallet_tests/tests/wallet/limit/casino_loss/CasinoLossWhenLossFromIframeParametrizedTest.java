...
class CasinoLossWhenLossFromIframeParametrizedTest extends BaseParameterizedTest {
    ...
    @DisplayName("Изменение остатка CasinoLossLimit при получении проигрыша от iframe")
    void shouldRejectBetWhenGamblingDisabled(NatsLimitIntervalType periodType) {
        ...
        final BigDecimal lossAmount = new BigDecimal("0");
        ...
        step("Manager API: Получение проигрыша", () -> {
            ...
            step("NATS: Проверка поступления события loosed_from_iframe", () -> {
                ...
                ctx.lossEvent = natsClient.findMessageAsync(
                        subject,
                        NatsBettingEventPayload.class,
                        filter).get();

                assertNotNull(ctx.lossEvent, "nats.event.loosed_from_iframe");
            });
        });
