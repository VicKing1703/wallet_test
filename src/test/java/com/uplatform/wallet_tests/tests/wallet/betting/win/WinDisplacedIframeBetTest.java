...
/**
 * Проверка win для вытесненной из Redis ставки.
 *
 * Совершается ряд iFrame ставок до превышения лимита, после чего win отправляется для вытесненной ставки.
 *
 * <p><b>Сценарий теста:</b></p>
 * <ol>
 *   <li><b>Регистрация игрока:</b> создание пользователя.</li>
 *   <li><b>Основное действие:</b> цикл ставок и win по вытесненной.</li>
...
class WinDisplacedIframeBetTest extends BaseTest {
    ...
    private static final BigDecimal winAmountForDisplacedBet = new BigDecimal("10.50");
    private static final BigDecimal winCoefficient = new BigDecimal("10.50");

    @Test
    @DisplayName("API выигрыш iFrame ставки вытесненной из Redis")
    void testApiWinByModifyingOriginalRequestForDisplacedIframeBet() {
        ...
        final class TestContext {
            ...
            Long lastMadeBetId;
            MakePaymentRequest originalBetRequestToProcessWin;
        ...
        step("Manager API: Совершение " + currentTransactionCountToMake + " iFrame ставок", () -> {
            for (int i = 0; i < currentTransactionCountToMake; i++) {
                var betInputData = MakePaymentData.builder()
                        ...
                        .totalCoef(winCoefficient.toString())
                        .build();
        ...
        step("Определение вытесненных iFrame ставок из Redis", () -> {
            ...
            var betIdForWin = displacedBetIds.iterator().next();

            ctx.originalBetRequestToProcessWin = ctx.madeBetsRequests.stream()
                    .filter(betReq -> Objects.equals(betReq.getBetId(), betIdForWin))
                    .findFirst()
                    .orElseThrow(() -> new AssertionError("test.displaced_request.not_found"));
        });

        step("Manager API: Выигрыш по вытесненной iFrame ставке (модификацией исходного запроса) и проверка ответа", () -> {
            var requestForWin = ctx.originalBetRequestToProcessWin;
            requestForWin.setType(NatsBettingTransactionOperation.WIN);
            requestForWin.setSumm(winAmountForDisplacedBet.toPlainString());
            requestForWin.setTotalCoef(winCoefficient.toString());
            requestForWin.setTime(System.currentTimeMillis() / 1000L);

            var response = managerClient.makePayment(requestForWin);
