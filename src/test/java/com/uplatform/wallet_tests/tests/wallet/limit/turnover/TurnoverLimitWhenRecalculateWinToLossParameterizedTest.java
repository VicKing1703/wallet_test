 ...
 * после совершения ставки на спорт (betting), последующего сообщения о выигрыше,
 * и затем перерасчета результата на проигрыш через Manager API.
 * Тест проверяет, что значения лимита (rest, spent) соответствуют ожидаемым после этих операций.
 * Важно: Первоначальный выигрыш и последующий перерасчет на проигрыш не должны изменять состояние лимита оборота,
 ...
 *      <li>Обработка выигрыша по ставке через Manager API ({@code /make-payment} для betting, операция WIN).</li>
 *      <li>Обработка перерасчета на проигрыш по ставке через Manager API ({@code /make-payment} для betting, операция LOSS).</li>
 *     </ul>
 *   </li>
 ...
 *   <li>{@code WIN} - первоначальный выигрыш по ставке.</li>
 *   <li>{@code LOSS} - проигрыш по ставке (в данном случае как результат перерасчета).</li>
 * </ul>
 */
...
class TurnoverLimitWhenRecalculateWinToLossParameterizedTest extends BaseParameterizedTest {
    ...
    @DisplayName("Проверка состояния TurnoverLimit после ставки, выигрыша и перерасчета на проигрыш")
    void shouldMaintainTurnoverLimitStateAfterBetWinAndRecalculationToLoss(NatsLimitIntervalType periodType) {
        ...
        step("Manager API: Перерасчет результата на проигрыш (LOSS)", () -> {
            ctx.betRequestBody.setSumm(lossAmount.toString());
            ctx.betRequestBody.setType(NatsBettingTransactionOperation.LOSS);

            var response = managerClient.makePayment(ctx.betRequestBody);
            assertEquals(HttpStatus.OK, response.getStatusCode(), "manager_api.make_payment(LOSS_recalculated).status_code");
