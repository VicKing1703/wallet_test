...
class TurnoverLimitWhenRefundParameterizedTest extends BaseParameterizedTest {
    ...
    void testTurnoverLimitChangeOnFullRefund(NatsLimitIntervalType periodType) {
        ...
        step("Manager API: Получение рефанда", () -> {
            ...
            assertAll("manager_api.refund.response_validation",
                    ...
                    () -> assertEquals(ctx.refundRequestBody.getTransactionId(), response.getBody().transactionId(), "manager_api.refund.body.transactionId"),
                    () -> assertEquals(0, ctx.expectedPlayerBalanceAfterRefund.compareTo(response.getBody().balance()), "manager_api.refund.body.balance")
            );
            ...
            step("Sub-step NATS: Проверка поступления события refunded_from_gamble", () -> {
                ...
                ctx.refundEvent = natsClient.expect(NatsGamblingEventPayload.class)
                    ...
                    .with("$.operation", NatsGamblingTransactionOperation.REFUND.getValue())
                    .fetch();
        ...
        step("Redis(Wallet): Проверка изменений лимита и баланса в агрегате ПОСЛЕ РЕФАНДА", () -> {
            ...
            assertAll("redis.wallet.limit_balance_after_refund",
                    ...
                    () -> {
                        ...
                        assertEquals(0, ctx.expectedRestAmountAfterRefund.compareTo(turnoverLimit.rest()), "redis.wallet.limit.rest");
                        assertEquals(0, ctx.expectedSpentAmountAfterRefund.compareTo(turnoverLimit.spent()), "redis.wallet.limit.spent");
