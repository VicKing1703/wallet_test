    ...
    void shouldRejectBetWhenCasinoLossLimitReached(NatsBettingCouponType couponType, String description) {
        ...
        step("Public API: Установка лимита на проигрыш", () -> {
            ...
            step("Sub-step NATS: получение события limit_changed_v2", () -> {
                ...
                assertAll("nats.limit_changed_v2_event.content_validation",
                        ...
                        () -> assertEquals(ctx.kafkaLimitMessage.expiresAt(), limit.expiresAt(), "nats.limit_changed_v2_event.limit.expiresAt"),
        ...
        step("Manager API: Совершение ставки на спорт", () -> {
            ...
            assertAll("Проверка статус-кода и тела ответа",
                    ...
                    () -> assertEquals(LOSS_LIMIT_REACHED.getDescription(), response.getBody().description(), "manager_api.body.description"),
                    () -> assertEquals(LOSS_LIMIT_REACHED.getCode(), response.getBody().errorCode(), "manager_api.body.errorCode")
            );
        });
    }
}