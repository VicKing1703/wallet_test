    ...
    void spentResetAfterPeriod(NatsLimitIntervalType periodType, int periodSeconds) throws Exception {
        ...
        step("Public API: Получение лимитов игрока", () -> {
            ...
            assertAll("fapi.get_casino_loss_limits.limit_content_validation",
                    () -> assertEquals(ctx.resetEvent.getPayload().getLimits().get(0).getExternalId(), fapiLimit.getId(), "fapi.get_casino_loss_limits.limit.id"),
                    ...
                    () -> assertEquals(0, LIMIT_AMOUNT.compareTo(fapiLimit.getAmount()), "fapi.get_casino_loss_limits.limit.amount"),
                    () -> assertEquals(0, LIMIT_AMOUNT.compareTo(fapiLimit.getRest()), "fapi.get_casino_loss_limits.limit.rest_equals_amount"),
