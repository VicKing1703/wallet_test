...
/**
 * Проверяет сброс потраченной суммы лимита CasinoLoss после истечения периода.
 *
 * Создаётся CasinoLoss лимит со временем начала, почти равным длительности периода
 ...
 *   <li><b>Основное действие:</b> установка лимита CasinoLoss с прошедшим startedAt.</li>
 ...
 *   <li><b>Проверка Public API:</b> получение лимитов CasinoLoss.</li>
 * </ol>
 *
...
class CasinoLossLimitSpentResetAfterPeriodParameterizedTest extends BaseParameterizedTest {
    ...
    @DisplayName("Сброс потраченной суммы casino-loss лимита после истечения периода")
    void spentResetAfterPeriod(NatsLimitIntervalType periodType, int periodSeconds) throws Exception {
        ...
        step("Redis(Wallet): Проверка данных лимита в агрегате", () -> {
            ...
            assertAll("redis.wallet_aggregate.limit_content_validation",
                    ...
                    () -> assertEquals(0, LIMIT_AMOUNT.compareTo(redisLimit.rest()), "redis.wallet_aggregate.limit.rest_equals_amount"),
        ...
        step("CAP: Получение лимитов игрока", () -> {
            ...
            var capLimitOpt = response.getBody().data().stream()
                    .filter(l -> LIMIT_AMOUNT.compareTo(l.amount()) == 0)
                    .findFirst();
            ...
            assertAll("cap.get_player_limits.limit_content_validation",
                    ...
                    () -> {
                        if (capLimit.spent() != null) {
                            assertEquals(0, BigDecimal.ZERO.compareTo(capLimit.spent()), "cap.get_player_limits.limit.spent_is_zero");
                        }
                    },
        ...
        step("Public API: Получение лимитов игрока", () -> {
            ...
            assertAll("fapi.get_casino_loss_limits.limit_content_validation",
                    () -> assertEquals(ctx.resetEvent.getPayload().getLimits().get(0).getExternalId(), fapiLimit.id(), "fapi.get_casino_loss_limits.limit.id"),
