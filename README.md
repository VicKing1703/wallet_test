...
Метод `.with(BiPredicate)` оставлен только для обратной совместимости и помечен
`@Deprecated`. Все новые проверки рекомендуется описывать через JSONPath и
метаданные — их проще комбинировать и они автоматически попадают в Search Info.
